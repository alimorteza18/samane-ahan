...
const Search = () => {
  ...
  const handleInput = (e: any) => {
    if (e.key === "Enter") {
      let url: { type?: string; kind?: string; size?: string } = {
        type: undefined,
        kind: undefined,
        size: undefined,
      };
      ...
      if (reg.test(str)) {
        ...
        str = str.replace(typeString, "");
      }
      if (regKind.test(str)) {
        kindString = str.match(regKind)[0];
        str = str.replace(kindString, "");
      }
      if (regSize.test(str)) {
        numberString = str.match(regSize)[0];
        str = str.replace(numberString, "");
      }

      if (typeString) {
        Object.values(PRODUCT_TYPE_DETAILS).map((types) => {
          Object.values(types).map((type) => {
            if (type == typeString) url.type = types.enLabel;
          });
        });
        ...
        if (kindString) {
          Object.values(PRODUCT_KIND_DETAILS).map((kinds) => {
            Object.values(kinds).map((kind) => {
              if (kind == kindString) url.kind = kinds.enLabel;
            });
          });
        }

        if (numberString) url.size = numberString;
      }

      if (url.type)
        router.push(
          `/price/${url.type}${url.kind ? "-" + url.kind : ""}${
            url.size ? "-" + url.size : ""
          }`
        );
